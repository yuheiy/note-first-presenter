# Plan 004: 公開前ハイジーン — package.json メタデータ・LICENSE・依存ピンを整える
...
> maintain the index.
>
> **Drift check (run first)**: `git diff --stat c4a6e81..HEAD -- packages/note-first-presenter/package.json packages/client/package.json pnpm-workspace.yaml`
...
また runtime 依存の `vite` が `npm:@voidzero-dev/vite-plus-core@latest` — `@latest` は公開パッケージの利用者側では**インストール時点の最新**に解決され、ロックファイルの保護もないため、CLI の挙動が利用者環境で勝手に変わりうる。`chokidar` だけカタログを経由せずリテラル指定になっている一貫性の問題もある。
...
- CLI の依存（`packages/note-first-presenter/package.json`）: `"chokidar": "^5.0.0"`（リテラル）、`"vite": "npm:@voidzero-dev/vite-plus-core@latest"`（catalog 経由）。他はすべて `"catalog:"`。
- `pnpm-workspace.yaml` の catalog 抜粋:
  ```yaml
  vite: npm:@voidzero-dev/vite-plus-core@latest # line 12
  vitest: npm:@voidzero-dev/vite-plus-test@latest # line 13
  vite-plus: latest # line 14
  chokidar: ^5.0.0 # line 29
  ```
- 現在ロックファイルで解決されているバージョン: `@voidzero-dev/vite-plus-core@0.1.24`（`pnpm-lock.yaml` で確認済み）。
...
- 依存の**アップグレード** — このプランは「現在動いているバージョンへのピン留め」のみ。新バージョンへの更新は別作業。

## Git workflow

- Branch: `advisor/004-publish-hygiene`
- Commit style: conventional commits（例: `chore: fix package metadata, add LICENSE, pin toolchain versions`）
...
`pnpm-workspace.yaml` の catalog を変更:

```yaml
vite: npm:@voidzero-dev/vite-plus-core@^0.1.24
vitest: npm:@voidzero-dev/vite-plus-test@<現在の解決バージョンに ^ を付けたもの>
vite-plus: <現在の解決バージョンに ^ を付けたもの>
```

`vitest` と `vite-plus` の現在の解決バージョンは `pnpm why vitest` / `pnpm why vite-plus`（または `pnpm-lock.yaml` の該当行）で確認してから書くこと。推測で書かない。

**Verify**: `vp install` → exit 0

### Step 5: 全テストで回帰がないことを確認する

ピンにより解決バージョンが `@latest` 時点から変わっていないなら挙動は同一のはず。フル検証する:
...
- [ ] `grep -n "@latest\|: latest" pnpm-workspace.yaml` がマッチしない
...
- `vp install` がピン変更後に失敗する、または解決バージョンが現在と大きく変わってテストが落ちる（ピンではなくアップグレードになってしまっている）— どのバージョンに解決されたかを添えて報告。
- `vite-plus` のピンで `vp` コマンド自体の挙動が変わった場合（グローバル CLI とローカル版の不整合）。
- e2e が環境要因（Playwright ブラウザ未導入等）で落ちる場合 — `playwright install` を 1 度試し、それでも落ちるなら失敗ログを添えて報告。

## Maintenance notes

- 以後の依存更新は catalog の 1 箇所を意図的に上げる運用になる（renovate/dependabot を入れるならこのファイルを対象にする）。
