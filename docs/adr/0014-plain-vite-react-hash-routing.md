...
- **`svelte.config.js` が消え、knip の SvelteKit プラグイン検出という前提も消えた。** 0007 の追記（2026-06-12）はこのファイルを「knip 検出専用に空のまま維持する」と決めていたが、自動提供されていた3つ（ルートの entry パターン / `$lib` 解決 / `$app`・`$env` の ignore）は React 構成では全て不要である。`knip.json` は client の entry を `src/main.tsx` 1本、project を `src/**/*.{ts,tsx}` と手書きする形に戻る。**代わりに knip が本来の仕事をするようになった** — 旧設定の `entry: ["src/**/*.svelte"]` は全コンポーネントをエントリ扱いにしており、未使用コンポーネントを1つも検出できていなかった。テストファイルは entry に列挙しない（Vitest プラグインが `vite.config.ts` の `test` 設定から自動的にエントリとして拾う）。
