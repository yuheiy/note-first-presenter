...
- **`vite` は catalog ではなく実 `vite` の範囲で宣言する（`^8.0.0`）。** ワークスペースの catalog は `vite` を `npm:@voidzero-dev/vite-plus-core` に別名解決しており、`catalog:` のままだと**その別名が公開マニフェストに焼き付く**。そして vite-plus-core は実行時に `require('vite-plus/binding')` でネイティブバインディングを取りにいくのに、`vite-plus` を dependencies にも peer にも optional にも宣言していない — 公開ユーザーの環境には存在しないので、**CLI は最初の一歩で `Cannot find native binding` で落ちる**。#38 とは無関係な、publish を阻む第二の障害だった。ローカルの解決は変わらない（`overrides: { vite: 'catalog:' }` が引き続き vite-plus-core に寄せる）ので、開発とテストはこれまで通り vite-plus-core を踏む。両者が入れ替え可能であることは、隔離環境で `build` した成果物のチャンクハッシュが完全一致することで確かめた。
- **`prepack` であって `prepublishOnly` ではない。** `prepublishOnly` は publish でしか走らないので、`pnpm pack` が **`dist/` の入っていない tarball を成功として出す**。`bin` と `package.json` だけの、起動すらしない成果物が黙って出来上がる。公開形態を検証するゲート（別 ADR）はまさに pack するので、ここを間違えるとゲートが「CLI が壊れている」という誤った診断を返す。`prepack` は pack と publish の両方で走る。
...
- **開発ループが2プロセスになる。** CLI は `vp pack --watch`、demo は `node --watch-path=<CLI の dist>` が変化を見て再起動する。client 側の編集は今まで通り HMR で即反映される（Vite が読んでいるので、そもそもビルドを挟まない）。
  ...
  - **demo の `dev` は `package.json` ではなく `demo/vite.config.ts` のタスクに置き、`note-first-presenter#build` を `dependsOn` に張る。** クローン直後は `dist/` が無く、`node --watch-path` は監視対象のディレクトリが存在しないと `ENOENT` で即死し、bin は `ERR_MODULE_NOT_FOUND` を投げる。どちらも「先にビルドしろ」とは言わない。root の `dev` はこれを `vp pack --watch` と並列に起動するので、宣言は初回の dist を巡る競合も同時に防ぐ。
