...
なお、ルーティングに関する判断は [ADR-0017](./0017-router-mode-base-and-wouter-boundaries.md) が置き換えた（`routerMode` + `base` + `?slide=`、そして wouter の境界付き導入 — この ADR の「ルータも入れない」「起動時に1回読む」「リスナを置かない」はルートについてはもう生きておらず、slide のミラーにだけ残る）。**「hash はサーバに届かない」という一文を根拠に何かを却下してはならない**（ADR-0017）。
