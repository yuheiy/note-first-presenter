...
本文は「`svelte.config.js` は存在しない」としていたが、コミット 2028f5a で空の
`packages/client/svelte.config.js`（`export default {};`）を意図的に再追加した。
knip の SvelteKit プラグインがこのファイルの存在でプロジェクトを検出し、ルートの
エントリパターン・`$lib` エイリアス・`$app`/`$env` の ignore を自動提供するためで、
これにより `knip.json` の手書き設定を削減できた。

このファイルは knip の検出専用であり、空のまま維持する。SvelteKit の実設定は
本文のとおりインラインで渡す方針に変更はない（CLI: `src/vite/index.ts` の
`createViteConfig`、client: `vite.config.ts` の `sveltekit({})`）。設定を
`svelte.config.js` に書き始めると、CLI が `configFile: false` で起動する経路と
二重管理になるため禁止する。

## 追記（2026-06-13）: Vite プラグインの配線先

dev/build は CLI の `createViteConfig` が担い client の `vite.config.ts` はテスト/IDE 専用なので、
