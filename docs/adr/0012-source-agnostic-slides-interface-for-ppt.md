...
- `SLIDES_EXTENSIONS`（現在 `['pdf']`）と `resolveSlides` の自動検出がソース追加時の変更点。複数ソースファイルが同居した場合の優先順位はその時点で決める。
