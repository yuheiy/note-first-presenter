...
- **実行時間はローカル 17 秒、CI 10 秒**（ローカルの内訳は pack 4.8 / install 7.0 / build 6.1 / export 0.8 / dev 1.3、pnpm ストア温）。CI の方が速いのは `setup-vp` の `cache: true` がストアを温めているためで、隔離ディレクトリへの install もハードリンクで済む — 「冷えたストアから 139 パッケージを引くので 1〜2 分」という事前見積もりは外れだった。`verify-package` は `test` job と並列に走るので、**CI 全体の実時間は増えていない**（1分50秒のまま）。ADR-0005 の追記 (c) にある「約60秒」は Playwright の setup project 全体の話で、`build` 単体ではない。
