...
- Every runtime dependency of `packages/note-first-presenter` must be in its own `dependencies` (`pdfjs-dist`, `@napi-rs/canvas`, `eta`). `vp check` cannot catch this: an undeclared import resolves from the hoisted workspace locally and breaks only for published users.
- A test file's name and location pick its layer: `test/*.test.ts` is CLI integration, `e2e/**/*.e2e.ts` is Playwright, a package's `*.browser.test.{ts,tsx}` is real Chromium, and any other `*.test.ts` is Node. `vp run test` runs all four (`docs/adr/0005`).
